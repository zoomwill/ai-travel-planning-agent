...
const plan = {
  ...
  flight: { flight_number: "MK318", airline: "Mock Pacific", origin: "Cleveland", destination: "Tokyo", departure_time: "2027-10-12T08:30:00Z", arrival_time: "2027-10-12T20:30:00Z", duration_minutes: 720, price: "900.00", currency: "USD" },
  hotel: { name: "Tokyo Quiet Hotel", city: "Tokyo", rating: 4.6, price_per_night: "180.00", currency: "USD", distance_to_center_km: 2.1, amenities: ["Wi-Fi", "breakfast"] },
...
async function installRoutes(page: Page): Promise<void> {
  ...
  await page.route("**/api/v1/agents/threads/*/conversation/confirm/stream", async (route) => {
    ...
    const events = [
      ...
      ...(["flights", "hotels", "attractions", "weather", "route"] as const).flatMap((kind, index) => [event("search_started", { search_kind: kind }, 7 + index * 2, "travel_search"), event("search_completed", { search_kind: kind, status: "ok", result_count: 1 }, 8 + index * 2, "travel_search")]),
      ...
      event("plan_completed", { travel_plan: plan, review_status: "accepted", review_rounds: 1, final_score: 91 }, 20, "finalize_plan"),
    ];
    planned = true;
